...
app.post('/signup', function (req, res) {
	MongoClient.connect(url, async function(err, client) {
		...
		app.users = await collection.find({"first": req.body.user.first,"last":req.body.user.last,"email":req.body.user.email,"password":req.body.user.password}).toArray()
...
app.post('/login', function(req,res){
	MongoClient.connect(url, async function(err, client) {
		 ...
		 const db = client.db(dbName);
		 const collection = db.collection('users');
		app.user = await collection.findOne({"email":req.body.user.email,"password":req.body.user.password}).then(doc=> 
		{
	...
	socket.on('join', function(){
		MongoClient.connect(url, async function(err, client) {
			...
			await collection.findOne({"_id":app.user._id}).then(doc=> 
			{
				...
				socket.broadcast.emit('announcement', doc.first + ' joined the chatroom.');
			});
		});
